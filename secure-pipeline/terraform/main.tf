...
# KMS Key for S3 and other encryption
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Action = "s3:*"
        Resource = [
          aws_s3_bucket.document_uploads.arn,
          "${aws_s3_bucket.document_uploads.arn}/*"
        ]
        Condition = {
          Bool = {
...
resource "aws_s3_bucket" "processed_storage" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "secure-pipeline-processed"
    }
  )
}
